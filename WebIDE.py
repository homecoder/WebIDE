...
if PYTHONISTA:
    ...
    ROOT = os.path.realpath(
        '..') + '/'  # Otherwise set the folder just above us (presumably the one that has all the stuff you want to edit)
...
def make_file_tree(dir_path=ROOT):  # Make a dict of the folder hierarchy (starting at ROOT if not specified)
    ...
    dir_path = dir_path.rstrip(
        '/')  # Get rid of those nasty slashes at the end of the string (for matching purposes later)
    ...
    def recur(path, list):  # Our recursive function to delve into the depths of the directory tree
        for l in os.listdir(path):  # For each file/folder in the current folder we're working on...
            ...
            elif l.split('.')[-1] in ['py', 'txt', 'pyui', 'json', 'tpl', 'html', 'md',
                                      'htm']:  # Or if we found a valid file...
                list[l] = pathname2url(f[len(
                    dir_path) + 1:])  # Then add the filename to the tree along with it's full pathname (relative to ROOT)
...
def run_script(filename):
    ...
    try:
        ...
        """
        rtdata = runpy.run_path(runscript)
        return True
    except Exception as e:
...
def edit():  # This function will get called for each GET request to /
    ...
    if filename:  # If there is a filename...
        fullname = os.path.realpath(
            os.path.join(ROOT, filename))  # Get the full path (relative to /, not ROOT this time)
        if os.path.isfile(fullname) and fullname.startswith(
                ROOT):  # If it's a file, and it is inside our ROOT directory (for safety)
            with open(fullname, encoding='utf-8', mode='r') as in_file:  # Open the file...
                code = in_file.read()  # And read all of it's juicy insides
                return template('./main.tpl', files=tree, filename=filename,
                                code=code)  # Read our template and give it the file tree, the name of the file, and the code
        else:  # If it's not a file or not in the ROOT directory...
            return template('./main.tpl', files=tree,
                            error='Invalid filename')  # Then yell at the user for being an idiot
...
def submit():  # This function will get called for each POST request to /
    ...
    if fullname.startswith(ROOT):  # If our file is in ROOT...
        ...
        if PYTHONISTA:  # And if we are in Pythonista
            ...
            # Added for Save & Run
            runfile = request.forms.get('runfile')
            if runfile == 'runfile':
                try:
                    if not web_ide_options['enable_run']:
                        return
                    run_data = run_script(filename)
...
def error404(code):  # This function will be called whenever there is a 404 error
    return template('./main.tpl', files=make_file_tree(ROOT),
                    error='This is not the page you\'re looking for *waves hand*')  # Guess what this does? Yep, yell at the user
...
if PYTHONISTA:  # If we are running in Pythonista...
    ...
    if ENABLE_BONJOUR:
        ...
        service = NSNetService.alloc().initWithDomain_type_name_port_('', '_http._tcp', 'Pythonista WebIDE',
                                                                      8080)  # Do ALL THE MAGICS!
